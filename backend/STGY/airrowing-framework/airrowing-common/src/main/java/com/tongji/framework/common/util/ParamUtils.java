...
public final class ParamUtils {
     ...
     * @return
     */
    public static boolean checkairrowingId(String airrowingId) {
        // 检查长度
        if (airrowingId.length() < ID_MIN_LENGTH || airrowingId.length() > ID_MAX_LENGTH) {
            return false;
        }
        // 检查格式
        Pattern pattern = Pattern.compile(ID_REGEX);
        return pattern.matcher(airrowingId).matches();
    }
