package com.tongji.airrowing.user.biz.service.impl;
...
import com.tongji.airrowing.user.biz.constant.RedisKeyConstants;
import com.tongji.airrowing.user.biz.constant.RoleConstants;
import com.tongji.airrowing.user.biz.domain.dataobject.RoleDO;
import com.tongji.airrowing.user.biz.domain.dataobject.UserDO;
import com.tongji.airrowing.user.biz.domain.dataobject.UserRoleDO;
import com.tongji.airrowing.user.biz.domain.mapper.RoleDOMapper;
import com.tongji.airrowing.user.biz.domain.mapper.UserDOMapper;
import com.tongji.airrowing.user.biz.domain.mapper.UserRoleDOMapper;
import com.tongji.airrowing.user.biz.enums.ResponseCodeEnum;
import com.tongji.airrowing.user.biz.enums.SexEnum;
import com.tongji.airrowing.user.biz.model.vo.UpdateUserInfoReqVO;
import com.tongji.airrowing.user.biz.rpc.DistributedIdGeneratorRpcService;
import com.tongji.airrowing.user.biz.rpc.OssRpcService;
...
public class UserServiceImpl implements UserService {
    ...
    public Response<?> updateUserInfo(UpdateUserInfoReqVO updateUserInfoReqVO) {
        ...
        if (StringUtils.isNotBlank(nickname)) {
            ...
            userDO.setNickname(nickname+userIdStr);
            needUpdate = true;
        }

        // 小哈书号
        String airrowingId = updateUserInfoReqVO.getAirrowingId();
        if (StringUtils.isNotBlank(airrowingId)) {
            Preconditions.checkArgument(ParamUtils.checkairrowingId(airrowingId), ResponseCodeEnum.AIRROWING_ID_VALID_FAIL.getErrorMessage());
            userDO.setAirrowingId(airrowingId);
            needUpdate = true;
        }
    ...
    public Response<Long> register(RegisterUserReqDTO registerUserReqDTO) {
        ...
        String airrowingId = distributedIdGeneratorRpcService.getairrowingId();
        ...
        UserDO userDO = UserDO.builder()
                .id(userId)
                .email(email)
                .airrowingId(airrowingId) // 自动生成小红书号 ID
                .nickname("小红薯" + userIdStr) // 自动生成昵称, 如：小红薯10000
     ...
     * 根据昵称查询用户信息
     *
     * @param findUserByNicknameReqDTO
     * @return
     */
    @Override
    public Response<FindUserByNicknameRspDTO> findByNickname(FindUserByNicknameReqDTO findUserByNicknameReqDTO) {
        String nickname = findUserByNicknameReqDTO.getNickname();

        // 根据昵称查询用户信息
        UserDO userDO = userDOMapper.selectByNickname(nickname);
        ...
        // 构建返参
        FindUserByNicknameRspDTO findUserByNicknameRspDTO = FindUserByNicknameRspDTO.builder()
                .id(userDO.getId())
                .password(userDO.getPassword())
                .build();

        return Response.success(findUserByNicknameRspDTO);
    }
     ...
     */
    @Override
    public Response<UserDO> getCurrentUserInfo() {
        // 获取当前登录用户的 ID
        Long currentUserId = LoginUserContextHolder.getUserId();

        // 从数据库查询用户信息
        UserDO userDO = userDOMapper.selectByPrimaryKey(currentUserId);
