...
import com.tongji.airrowing.count.biz.model.dto.NoteOperateMqDTO;
...
@Component
@RocketMQMessageListener(consumerGroup = "airrowing_group_" + MQConstants.AIR_TOPIC_NOTE_OPERATE, // Group 组
        topic = MQConstants.AIR_TOPIC_NOTE_OPERATE // 主题 Topic
)
@Slf4j
public class CountNotePublishConsumer implements RocketMQListener<Message> {
    ...
    public void onMessage(Message message) {
        ...
        // 根据 MQ 标签，判断笔记操作类型
        if (Objects.equals(tags, MQConstants.AIR_TAG_NOTE_PUBLISH)) { // 笔记发布
            handleTagMessage(bodyJsonStr, 1);
        } else if (Objects.equals(tags, MQConstants.AIR_TAG_NOTE_DELETE)) { // 笔记删除
            handleTagMessage(bodyJsonStr, -1);
        }
    }
