...
@Component
@RocketMQMessageListener(consumerGroup = "airrowing_group_" + MQConstants.AIR_TOPIC_COLLECT_OR_UN_COLLECT, // Group 组
        topic = MQConstants.AIR_TOPIC_COLLECT_OR_UN_COLLECT, // 消费的主题 Topic
...
public class CollectUnCollectNoteConsumer implements RocketMQListener<Message> {
    ...
    public void onMessage(Message message) {
        ...
        // 根据 MQ 标签，判断操作类型
        if (Objects.equals(tags, MQConstants.AIR_TAG_COLLECT)) { // 收藏笔记
            handleCollectNoteTagMessage(bodyJsonStr);
        } else if (Objects.equals(tags, MQConstants.AIR_TAG_UN_COLLECT)) { // 取消收藏笔记
    ...
    private void handleCollectNoteTagMessage(String bodyJsonStr) {
        ...
        // 异步发送 MQ 消息
        rocketMQTemplate.asyncSend(MQConstants.AIR_TOPIC_COUNT_NOTE_COLLECT, message, new SendCallback() {
            @Override
