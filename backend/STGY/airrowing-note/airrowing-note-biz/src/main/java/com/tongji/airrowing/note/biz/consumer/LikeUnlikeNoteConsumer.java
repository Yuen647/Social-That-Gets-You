...
@Component
@RocketMQMessageListener(consumerGroup = "airrowing_group_" + MQConstants.AIR_TOPIC_LIKE_OR_UNLIKE, // Group 组
        topic = MQConstants.AIR_TOPIC_LIKE_OR_UNLIKE, // 消费的主题 Topic
...
public class LikeUnlikeNoteConsumer implements RocketMQListener<Message> {
    ...
    public void onMessage(Message message) {
        ...
        // 根据 MQ 标签，判断操作类型
        if (Objects.equals(tags, MQConstants.AIR_TAG_LIKE)) { // 点赞笔记
            handleLikeNoteTagMessage(bodyJsonStr);
        } else if (Objects.equals(tags, MQConstants.AIR_TAG_UNLIKE)) { // 取消点赞笔记
    ...
    private void handleLikeNoteTagMessage(String bodyJsonStr) {
        ...
        // 异步发送 MQ 消息
        rocketMQTemplate.asyncSend(MQConstants.AIR_TOPIC_COUNT_NOTE_LIKE, message, new SendCallback() {
            @Override
